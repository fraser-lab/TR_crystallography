...
def calculate_extrapolated_F_as_dict( ground_state_Fobs_as_text_fullpath, excited_state_Fobs_as_text_fullpath, extrapolation_factor ):
...
def run_phenix_refinement_with_SA( model, data ):

  refinement_command = "phenix.refine %s %s strategy=individual_sites+individual_adp refinement.main.simulated_annealing_torsion=True refinement.main.number_of_macro_cycles=7 refinement.main.nproc=8 refinement.target_weights.optimize_xyz_weight=True refinement.target_weights.optimize_adp_weight=True" % (model, data) # add weight opt??
...
def main():
  ...
  for extrapolation_factor in extrapolation_factors:
    ...
    extrapolated_F_dict = calculate_extrapolated_F_as_dict(scaled_ground_F_as_text_fullpath, scaled_excited_F_as_text_fullpath, extrapolation_factor)
    ...
    run_phenix_refinement_with_SA(ground_state_pdb_filename, data_filename_for_refinement)
